...
import { createStaticServer, parsePort } from '../server.js';
...
test('health reports v7 JSON', async () => { const server=await start(); try { const response=await request(server,'/health'); assert.equal(response.status,200); assert.equal(response.headers['content-type'],'application/json; charset=utf-8'); assert.deepEqual(JSON.parse(response.body),{ok:true,version:'v7'}); } finally { await stop(server); } });
